...
const MyAppointments = () => {
  ...
  const slotDateFormatter = (slotDate) => {
    ...
    const dd = String(Number(day)) // remove leading zeros if any
    const mm = String(month).padStart(2, '0') // ensure two digits for month
    return `${dd} tháng ${mm}, ${year}` // example: "1 tháng 02, 2025"
  }
        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex-1 text-sm text-zinc-700'>
              ...
              <p className='text-sm mt-1'><span className='text-sm text-neutral-700 font-medium'>Thời gian:</span> {slotDateFormatter(item.slotDate)} | {item.slotTime}</p>
            </div>
            <div></div>
